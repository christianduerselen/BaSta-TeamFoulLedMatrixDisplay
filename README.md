...
The foul count received from a Stramatel scoreboard controller is rendered in white using the "Bebas Neue" font. When the team reaches the bonus foul limit and the game clock is running, the entire display turns solid red. A push-button selects whether the home or guest team fouls are shown. If no data is received for 3 seconds the display clears automatically.
...
| Team selection button | Pin 13 (active low, internal pull-up) |
...
1. Wire the LED matrix and a momentary push-button to the Arduino Zero as described above.
...
- Press the **team selection button** to switch between displaying home and guest team fouls.
